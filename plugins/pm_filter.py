...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         text=f"__𝖧𝖾𝗒 Yᴏᴜʀ ᴍᴇssᴀɢᴇ ʜᴀs ʙᴇᴇɴ sᴇɴᴛ ᴛᴏ ᴍʏ ᴍᴏᴅᴇʀᴀᴛᴏʀs. Click on the **Request Here** button Below for getting the series.__",   
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📝 ʀᴇǫᴜᴇsᴛ ʜᴇʀᴇ ", url=f"https://t.me/+KqYvuXINs9cwMDdl")]])
    )
...
async def next_page(bot, query):
    ...
    #     ]
        ...
        btn.insert(0, [
            InlineKeyboardButton(f'‼️ Hᴏᴡ Tᴏ Usᴇ Bᴏᴛ ‼️', 'select')
        ])
    else:
        btn = []
    try:
    ...
    if settings["button"]:
        cap = f"<b>📁 Found ✨ Files For Your Query : #{search} 👇</b>"
    else:
    ...
    try:
        if int(query.from_user.id) not in [query.message.reply_to_message.from_user.id, 0]:
            return await query.answer(
                f"⚠️ ʜᴇʟʟᴏ{query.from_user.first_name},\nᴛʜɪꜱ ɪꜱ ɴᴏᴛ ʏᴏᴜʀ Series ʀᴇQᴜᴇꜱᴛ,\nʀᴇQᴜᴇꜱᴛ ʏᴏᴜʀ'ꜱ...",
                show_alert=True,
            )
    except:
        pass
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        try:
            if (AUTH_CHANNEL or REQ_CHANNEL) and not await is_subscribed(client, query):
                if clicked == typed:
                    ...
                    await query.answer(f"Hᴇʏ {query.from_user.first_name}, Tʜɪs Is Nᴏᴛ Yᴏᴜʀ Series Rᴇǫᴜᴇsᴛ. Rᴇǫᴜᴇsᴛ Yᴏᴜʀ's !", show_alert=True)
        ...
        InlineKeyboardButton('♻️ Group', url ='https://t.me/+KqYvuXINs9cwMDdl')
    ]]
        ...
        await query.message.edit_text(
            text=script.ALL_FILTERS.format(message.from_user.mention),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
